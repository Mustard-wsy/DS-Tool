...
> 注意：PageAgent **默认启用**（页面打开即初始化，面板收起，点工具栏「页面 AI 助手」按钮打开对话），在本地 `file://` 下同样可用、零报错。若同时配置 API URL、Token、模型，则使用用户自己的 OpenAI-compatible API；否则使用 PageAgent 默认测试接口（开箱即用，但可用性/额度不保证）。离线时 PageAgent 静默不可用，页面本身完全正常。生产环境不要把真实密钥直接放在前端页面里，应通过后端代理托管密钥和权限控制。

---

## 打开方式与输出

DSVis 生成**自包含 HTML**（G6 库、样式全部内联），用 `file://` 直接打开，无需任何本地服务，离线也能完整渲染。每次运行会覆盖输出到 `<cwd>/.dsvis/out/<脚本名>.html`，可长期保存、分享（发给别人双击打开）、刷新看最新。样式自动保存到 `<cwd>/.dsvis/<脚本名>.json`（需 Python 自动拉起的小型本地服务，用户无感）；本地服务不可用时自动落到浏览器 localStorage，页面照常工作。
...
DSVis 页面内置了 PageAgent（**默认启用**）：

- 页面打开即完成初始化，点击工具栏中的「页面 AI 助手」按钮打开对话面板。
- 在设置面板中可填写：
  - API URL
  - Token
  - 模型名
