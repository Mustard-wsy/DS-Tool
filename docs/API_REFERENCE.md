...
# 程序结束時，生成 template.html，包含 3 个步骤
```
...
### 7. `dsvis.bind_lists()` / `@dsvis.bind_lists()`
...
| 灵活性 | 高 | 低 | 中等 |
| 性能开销 | 低 | 中等 | 低 |
| 最终界面 | template.html | template.html | template.html |
...
| 接口 | 说明 | 替代方案 |
|-----|------|----------|
| `bind_field()` | 单项绑定冗余，被 `bind_fields()` 替代 | 使用 `dsvis.bind_fields()` 批量绑定 |
| `layout=None` 参数 | 不再需要显式传递 | 系统自动处理布局 |
| `title` 参数 | 不再需要显式传递步骤标签 | 系统自动生成标题 |
| `tag` 参数 | 不再需要显式传递步骤标签 | 系统自动生成标题 |
